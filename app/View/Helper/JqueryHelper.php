...
class JqueryHelper extends FormHelper {
     ...
     */
    function tabs($selector, $options = array()) {
       
        
        $var = strtr($selector,array('.'=>'','#'=>''));
        if (isset($options['var'])) {
            $var = 'var ' . $options['var']  . ' = ';
            unset($options['var']);
        } else {
        ...
        $script = "{$var} $('$selector').tabs($options);
        
        $('$selector a').click(function()".'{$(".fancybox-inner").height($(window).height()-100);}'.");";
        return $this->Javascript->codeBlock($script);
    }
	...
	/**
	 * Makes a datepicker input. Change commented line for datepicker.
	 *
	 * @param string $field Field name of the CakePHP Model
	 * @param array $phpOptions of cakephp options for form elements
	 * @param array $jsOptions of javascript options for jQuery
	 * @param boolean $masked if true changes the datepicker behaviour for a masked input
	 * @return string html+script code
	 */
    function date($field, $phpOptions = array(), $jsOptions = array(),$masked=false) {
        $id = $field . '_' . rand(10000, 99999);

        $phpOptions = array_merge(array(
            'type' => 'text',
            'class' => 'ui-state-default ui-widget ui-widget-input ui-corner-all',
            'id' => $id,
          //  'dateFormat'=>'DMY',
          //  'separator'=>'/',
                ),$phpOptions);
        ...
        $return = $this->Form->input($field, $phpOptions);
		/* MASKED INPUT */
		if($masked){
			$script = $this->Javascript->codeBlock($this->maskedInput($id));
		}else{
			$script = $this->datepicker($phpOptions['id'], $jsOptions);
		}
	...
	/**
	 * Makes a maskedinput control
	 *
	 * @param string $id DOM ID of maskedinput handle
	 * @param string $mask mask format to apply
	 * @return string script
	 * http://digitalbush.com/projects/masked-input-plugin/
	 */
    public function maskedInput($id,$mask="99/99/9999"){
        return "$(\"#$id\").keypress(function(event){
             ...
             if(event.which == 13 &&  $(this).val().length <=2 ){
                ...
                var yyyy = today.getFullYear();
                    $(this).val(dd+'/'+mm+'/'+yyyy);
                    $(this).parents().nextAll('div:first').find('input').focus(); 
                    return false;
             }else{
	...
	/**
	 * JqueryHelper Wrapper for 'postLink' function of FormHelper
	 */
	...
	/**
	 * JqueryHelper Wrapper for 'end' function of FormHelper
	 */
    ...
    /**
     * jQuery UI styled input
     *
     * Automagically detects the input type and creates smart inputs
     *
	 * @param string $fieldName Field name of the CakePHP Model
	 * @param array $options Cakephp options for form elements
	 * @return string html+script code
     */
    public function input($fieldName, $options = array()) {
        ...
        $fieldDef = $this->_introspectModel($modelName,'fields',$fieldKey);

       
        switch ($fieldDef['type']) {
            case 'date':
                
                $return = $this->date($fieldName, $options);
            ...
            default:
                ...
                if (is_array($varOptions)) {
                    if(isset($options['multiple']) && $options['multiple']==true){
                        ...
                        $return = $this->combobox($fieldName, $options);
                    }
                }else{
                    $return = $this->Form->input($fieldName, $options);
                }
	 ...
	 * @return string html+script code
	 */
    public function multiple($field,$options=array()) {
        $id = $field . '_' . rand(10000, 99999);
        $defaultPhp = array(
            'id' => $id,
           
        );
        $options= array_merge($defaultPhp,$options);
	...
	/**
	 * Creates a combobox with autocompletion.
	 *
	 * @param string $field Field name of the CakePHP Model
	 * @param array $options Cakephp options for form elements
	 * @return string html+script code
	 */
    public function combobox($field,$options=array()) {
        $id = $field . '_' . rand(10000, 99999);
        $defaultPhp = array(
            'id' => $id,
            'empty' => array(0 => '')
        ...
        $return = $this->Form->input($field, $options);
        $script = $this->Javascript->codeBlock("$('#".$options['id']."').combobox();");
        return $return . $script;
    }

                        
    /**
     * Opens an url in a fancy box
     *
     * Calls a remote action defined by <i>options[url]</i>
     * (using the url() format) that's called in the background using
     * XMLHttpRequest. The result is opened in a fancy box,
     *
     *
     * @param string/array $href Href string "/products/view/12" or CakePHP routing array.
     * @param array $options Options for the ajax call
	 * @param boolean $script If true returns the javascript code inside <script> tags.
     *
     * @return string HTML code for fancybox
     */
    function fancyBox($href, $options=array(), $script=true) {
        if (!isset($options['url'])) {
            $options['url'] = $href;
        ...
        $return = $this->remoteFunction($options);
    ...
    /**
     * Creates a link to be opened in a fancy box
     *
     * @param string $title text of the link.
     * @param string/array $href URL or CakePHP routing array
	 * @param array $options CakePHP options for links.
	 * @param boolean $confirm CakePHP confirm option for links.
	 * @param boolean $escapeTitle CakePHP escape htrml option for links.
     *
     * @return string HTML code for fancybox
     */
    function fancyLink($title, $href = null, $options = array(), $confirm = null, $escapeTitle = true) {
        if (!isset($href)) {
            $href = $title;
        }
        if (!isset($options['url'])) {
        ...
        $htmlOptions['onclick'] .= ' return false;';
        $return = $this->Html->link($title, $href, $htmlOptions, null, $escapeTitle);
        $callback = $this->fancyBox($href, $options, false) . ";\n return false;";
        $script = $this->Javascript->event("'#{$htmlOptions['id']}'", "click", $callback);
	...
	/**
	 * Wrapper for 'remoteFunction' function
	 * This function creates the javascript needed to make a remote call
	 *
	 * @param array/string $href URL or CakePHP routing array
	 * @param array $options options for javascript
	 * @return string html code for link to remote action
	 */
    function load($href = null, $options = array(),$block=true) {
        if($href==null){
            return '';
        }
        if (!isset($options['url'])) {
        ...
        $htmlDefaults = array('id' => 'link' . intval(mt_rand()), 'onclick' => '');
        $htmlOptions = array_merge($htmlDefaults, $htmlOptions);
        $script = $this->remoteFunction($options);
		if($block){
			$script = $this->Javascript->codeBlock($script);
		}
